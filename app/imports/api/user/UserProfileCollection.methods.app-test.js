...
//       };
//       let docID = await defineMethod.callPromise({ collectionName, definitionData });
//       expect(UserProfiles.isDefined(docID)).to.be.true;
//       const sdoc = UserProfiles.findDoc(docID);
//       expect(sdoc.email).to.equal(definitionData.email);
//       expect(sdoc.firstName).to.equal(definitionData.firstName);
//       expect(sdoc.lastName).to.equal(definitionData.lastName);
//       const updateData = {
//         email: faker.internet.email(),
//         firstName: faker.name.firstName(),
//         lastName: faker.name.lastName(),
...
//       const doc = UserProfiles.findDoc(docID);
